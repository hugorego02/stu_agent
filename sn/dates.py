...
def _utc_range(keyword: str) -> Tuple[str, str]:
    ...
    if k in ("today",):
        ...
        wd = now.weekday()
        start = (now - timedelta(days=wd)).replace(hour=0, minute=0, second=0, microsecond=0)
        end = now
    elif k in ("last week",):
        wd = now.weekday()
        ...
        start = last_q_end.replace(
            year=q_start_year, month=q_start_month, day=1,
...
def _date_on_clause(field: str, keyword: str) -> str:
    """Return ONToday or ONYesterday clause if applicable."""
    k = keyword.strip().lower()
    if k == "today":
        return f"{field}ONToday"
    if k == "yesterday":
        return f"{field}ONYesterday"
    return ""
def parse_date_expression(text_l: str):
    period = None
    date_field = None
    filters = {}
